  ...
  <el-main class="bg table_wrap">
      ...
      <el-form-item>
        ...
        <el-button>
          <router-link
            v-if="
              user_group == '管理员' ||
              $check_action('/registered_users/table', 'add') ||
              $check_action('/registered_users/view', 'add')
            "
            to="./view?"
            >添加
          </router-link>
        ...
        <el-button
          v-if="
            user_group == '管理员' ||
            $check_action('/registered_users/table', 'del') ||
            $check_action('/registered_users/view', 'del')
          "
    ...
    >
      ...
      <el-table-column
        ...
        @sort-change="$sortChange"
        label="性别"
        v-if="user_group == '管理员' || $check_field('get', 'gender')"
        min-width="200"
        align="center"
      >
      </el-table-column>
      ...
      <el-table-column
        ...
        v-if="
          user_group == '管理员' ||
          $check_action('/registered_users/table', 'set') ||
          $check_action('/registered_users/view', 'set') ||
          $check_action('/registered_users/view', 'get') ||
...
export default {
  ...
  data() {
    return {
      ...
      // 详情路径
      viewPath: "/registered_users/view",
  ...
  methods: {
    // 单独写该方法,因为没行数据有两个id为: registered_users_id, user_id
    handleUpdate() {
      this.reset();
      const id = this.ids[0][this.viewId];
      this.$router.push({ path: this.viewPath, query: { [this.viewId]: id } });
    },
    // 关闭弹框
    closeModal() {
