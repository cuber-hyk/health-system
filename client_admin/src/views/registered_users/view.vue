    ...
    >
      ...
      >
        <el-form-item label="用户姓名" prop="user_name">
          <el-input
            id="user_name"
            v-model="form_sub['user_name']"
            placeholder="请输入用户姓名"
            v-if="
              user_group === '管理员' ||
              (form_sub['registered_users_id'] &&
                $check_field('set', 'user_name')) ||
              (!form_sub['registered_users_id'] &&
                $check_field('add', 'user_name'))
            "
      ...
      <el-col
        v-if="
          user_group === '管理员' ||
          $check_field('get', 'gender') ||
          $check_field('add', 'gender') ||
          $check_field('set', 'gender')
        "
        :xs="24"
        :sm="12"
        :lg="8"
      ...
      >
        <el-form-item label="性别" prop="gender">
          ...
          <div v-else-if="$check_field('get', 'gender')">
            {{ form_sub["gender"] }}
          </div>
        </el-form-item>
      </el-col>
...
export default {
  ...
  methods: {
    ...
    submit_sub(form_sub) {
      form_sub =
        this.events("submit_before", Object.assign({}, form_sub)) || form_sub;
      if (form_sub["registered_users_id"]) {
        // 提交事件
        this.$post(
          "~/api/" +
            this.group_table +
            "/set?registered_users_id=" +
            form_sub["registered_users_id"],
          form_sub,
          (res) => {
