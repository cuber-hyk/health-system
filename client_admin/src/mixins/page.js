export default {
  data: function data() {
    ...
    return {
      ...
      // 选中行对应数据的id(比如article_id, user_id)
      ids: [],
  ...
  methods: {
    ...
    set: function set(param, query, func, includeZero) {
      ...
      var pm =
        this.events("set_before", Object.assign({}, param), includeZero) ||
        param;
    ...
    set_before: function set_before(param, includeZero) {
      ...
      for (var k in pm) {
        if (
          k.toLocaleLowerCase().indexOf("time") !== -1 &&
          pm[k].indexOf("T") !== -1
        ) {
    ...
    get_list: function get_list(param, func) {
      ...
      var pm =
        this.events("get_list_before", Object.assign({}, param)) || param;
    ...
    set_main: function set_main(value, func) {
      ...
      this.$post(this.toUrl(this.query_set, url), value, function(
        json,
        status
      ) {
    ...
    // 修改数据 2024-11-02
    // 在table.vue页面data中添加viewPath,field
    handleUpdate() {
      this.reset();
      const id = this.ids[0][this.field];
      this.$router.push({ path: this.viewPath, query: { [this.field]: id } });
    },
     ...
     */
    selectionChange(val) {
      // 获取每个对象中字段名含有"id"字样的值
      const ids = val.map((obj) => {
