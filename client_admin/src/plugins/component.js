...
import page_side_vertical from "../components/common/page_side_vertical.vue";
import page_tag from "../components/common/page_tag.vue";
...
export default {
  ...
  install(Vue, optons) {
    ...
    Vue.component("PageSideVertical", page_side_vertical);
    Vue.component("pageTag", page_tag);
      ...
      methods: {
        ...
        $is_tel(rule, value, callback) {
          if (
            ...
            /^\d{3}-\d{7,8}|\d{4}-\d{7,8}|\d{8}$/.test(value)
          ) {
            callback();
          } else {
        ...
        $is_url(rule, value, callback) {
          if (
            !value ||
            /^([hH][tT]{2}[pP]:\/\/|[hH][tT]{2}[pP][sS]:\/\/|www\.)(([A-Za-z0-9-~]+)\.)+([A-Za-z0-9-~\/])+$/.test(
              value
            )
          ) {
            callback();
