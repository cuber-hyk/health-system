<template>
  <div class="page_root" id="root_demo">
    ...
    <!-- 推荐食物信息模块(开始) -->
    <div class="warp" v-if="$check_action('/food_information/list', 'get')">
      <!-- 容器 -->
      <div class="container">
        <div class="row">
          <div class="col-12">
            ...
            <list_food_information
              v-if="show_list_food_information"
              :list="list_food_information"
            />
            <div class="overflow-auto" v-else>
              <table
                id="list_diy"
              ...
              >
                ...
                <tbody>
                  <tr
                    v-for="(o, i) in list_table_food_information"
                    :key="i"
                    @click="
                      to_details('food_information', o, 'food_information_id')
                    "
                  >
    ...
    <!-- 推荐食物信息模块(结束) -->
    <!-- 推荐食谱信息模块(开始) -->
    <div class="warp" v-if="$check_action('/recipe_information/list', 'get')">
      <!-- 容器 -->
      <div class="container">
        <div class="row">
          <div class="col-12">
            ...
            <div class="diy_recommend">
              ...
              <div class="more_box">
                <router-link to="/recipe_information/list" class="more">
                  <span>More</span>
                </router-link>
              </div>
            </div>
            <!-- 食谱信息推荐列表组件 -->
            <list_recipe_information
              v-if="show_list_recipe_information"
              :list="list_recipe_information"
            />
            <div class="overflow-auto" v-else>
              <table
                id="list_diy"
                role="table"
                aria-busy="false"
                :aria-colcount="fields_recipe_information.length"
                class="table b-table table-striped table-hover"
              >
                <thead>
                  <tr>
                    <th v-for="(o, i) in fields_recipe_information" :key="i">
                      {{ o.label }}
                    </th>
                  </tr>
                </thead>
                <tbody>
                  <tr
                    v-for="(o, i) in list_table_recipe_information"
                    :key="i"
                    @click="
                      to_details(
                        'recipe_information',
                    ...
                    "
                  >
                    <td v-for="(oj, n) in fields_recipe_information">
                      <img
                        v-if="oj.type && oj.type == '图片'"
                        :src="$fullUrl(o[oj.key])"
                        alt=""
...
import list_recipe_information from "@/components/diy/list_recipe_information.vue";
...
export default {
  ...
  data() {
    return {
      ...
      fields_food_information: [
        ...
        {
          key: "food_introduction",
          label: "食物介绍",
          type: "编辑",
        },
      ],
      show_list_recipe_information: true,
      list_recipe_information: [],
      vm_arr_recipe_information: [
        "recipe_information_id",
        "recipe_name",
  ...
  methods: {
    get_token() {
      ...
      if (token) {
        ...
        location.href = "http://60.204.223.60:5002/";
      }
    },
    // toggle
    toToggle() {
    ...
    get_recipe_information() {
      let url = "~/api/recipe_information/get_list?";
      url = "~/api/recipe_information/get_hits_list?";
      this.$get(
        url,
        {
        ...
        (json) => {
          if (json.result) {
            this.list_recipe_information = json.result.list;
          }
        }
      );
    },
  ...
  computed: {
    ...
    list_table_recipe_information() {
      var list = this.list_recipe_information;
      var list_table = [];
      for (let i = 0; i < list.length; i++) {
        list_table[i] = {};
        this.vm_arr_recipe_information.map((o) => {
          // 第二个中括号是对象的属性
          list_table[i][o] = list[i][o] || "";
        });
      }
